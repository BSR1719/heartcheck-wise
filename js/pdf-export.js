...
function buildOnePageReport(data){
  ...
  report.innerHTML=`
    ...
    ${meaningHtml(data)}
    <section class="r-grid"><div class="r-card r-priority-card"><h2>สิ่งที่ควรใส่ใจก่อน</h2>${priorityHtml(data.priorities)}</div><div class="r-card r-plan"><h2>แผนของคุณใน 90 วัน</h2>${timelineHtml(data.timeline)}</div></section>
...
window.HeartCheckPdfExport={ensurePdfButton,downloadResultPdf,buildPdfBlob,deliverPdf,expandForPdf,ensurePdfLibraries,reportData,buildOnePageReport,riskMeaning,countRange,complementRange,riskCountLabel};
})();
